...
public final class Analyser {
    ...
    //  */
    // private void addSymbol(String name, boolean isInitialized, boolean isConstant, Pos curPos, SymbolType type) throws AnalyzeError {
    ...
    //  * @throws AnalyzeError 如果重复定义了则抛异常
    //  */
    // private void addGlobal(String name, boolean isInitialized, boolean isConstant, Pos curPos, SymbolType type) throws AnalyzeError {
    //     if (this.globalTable.get(name) != null) {
    //         throw new AnalyzeError(ErrorCode.DuplicateDeclaration, curPos);
    //     } else {
    //         this.globalTable.put(name, new SymbolEntry(isConstant, isInitialized, getNextVariableOffset(),type));
    //     }
    // }
    ArrayList<TokenType> options = new ArrayList<>(Arrays.asList(TokenType.GT,
     ...
     * @throws AnalyzeError 如果重复定义了则抛异常
     */
    private void addSymbol(String name, boolean isInitialized, boolean isConstant,  SymbolType symbolType, SymbolRange symbolRange, Pos curPos) throws AnalyzeError {
        if(this.hash.get(name)==null)//没有出现过
        {
            if(symbolRange == SymbolRange.Global)
            {
                this.symbolTable.push(new SymbolEntry(name, isConstant, isInitialized, symbolType, symbolRange, globalOffset++));
                if (isConstant)
                    Globals.add("1");
                else
                    Globals.add("0");
            ...
            {
                this.symbolTable.push(new SymbolEntry(name, isConstant, isInitialized, symbolType, symbolRange, localOffset++));
            }
            else if(symbolRange == SymbolRange.Param)
            {
                this.symbolTable.push(new SymbolEntry(name, isConstant, isInitialized, symbolType, symbolRange, paramOffset++));
            }
            else
            {
...
//开始语法分析
    ...
    private void analyseProgram() throws CompileError {
        ...
        Globals.add("_start");
        FunctionEntry s = new FunctionEntry(0,0,0,globalOffset++);
        funcOffset++;
        start.add(s);
    ...
    private void analyseFunction() throws CompileError {
        ...
        SymbolEntry s = new SymbolEntry(funcname.getValueString(), true, SymbolRange.Global, globalOffset++, funcOffset++);
        this.symbolTable.push(s);
     ...
     * block_stmt -> '{' stmt* '}'
     */
    private boolean[] analyseBlock_stmt(boolean isFunction, boolean isloop, SymbolType returntype, int loopaddr, ArrayList<Integer> breakaddrs) throws CompileError {
     ...
     *  | empty_stmt
     */
    private boolean[] analyseStmt(boolean isloop, SymbolType returntype, int loopaddr, ArrayList<Integer> breakaddrs) throws CompileError {
        //decl_stmt -> let_decl_stmt | const_decl_stmt
        ...
        else if(peek().getTokenType() == TokenType.IDENT)
        {
            ...
            else if(peek().getTokenType() == TokenType.L_PAREN)
            {
                ...
                else
                {
                    ...
                    else if(token.getValueString().equals("putchar")||token.getValueString().equals("putint")||token.getValueString().equals("putstr"))
                    {
    ...
    public void output(File output)throws IOException 
    {
        ...
        {
            if (instructions.get(i).getClass().isAssignableFrom(FunctionEntry.class))
            {
                funcs.add(new ArrayList<>(instructions.subList(startaddr, i)));
                startaddr = i;
            }
        }
        ...
        for (ArrayList<Object> funcins : funcs) {
            for (Object object : funcins) {
                ...
                {
                    Instruction ain = (Instruction) object;
